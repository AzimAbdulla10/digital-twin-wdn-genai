...
export const AIAlertCard: React.FC<AIAlertCardProps> = ({
  ...
  return (
    <div
      className={`w-full rounded-lg border p-5 flex flex-col transition-all duration-500 ${
        isLeak
          ? 'bg-[#000000]/90 border-red-500/40'
          : isDemandSurge
          ? 'bg-[#000000]/90 border-amber-500/30'
          : 'bg-[#000000]/90 border-emerald-500/30'
      }`}
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-zinc-100">AI Diagnostic & Disambiguation</h3>
      ...
      {/* Main Metric Banner */}
      <div className="flex items-center justify-between p-3.5 rounded-md bg-[#09090b]/80 border border-zinc-800/80 mb-3">
        <div className="flex items-center gap-3">
          {isLeak ? (
            <div className="p-2 rounded-md bg-red-500/20 text-red-400 border border-red-500/30">
      ...
      {disambiguation && (
        <div className="p-2.5 rounded-md bg-[#09090b]/60 border border-zinc-800/80 text-xs mb-3">
          ...
          <div className="mt-1.5 pt-1.5 border-t border-zinc-800 flex items-center gap-1.5 text-[11px] text-cyan-300 font-mono">
      ...
      {riskAssessment && (
        <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
          <div className="bg-[#09090b]/70 p-2 rounded-md border border-zinc-800">
            <span className="text-zinc-400 text-[10px] block">24h Predicted Min Head</span>
            <span
          ...
          <div className="bg-[#09090b]/70 p-2 rounded-md border border-zinc-800">
            <span className="text-zinc-400 text-[10px] block">Tank 2 Reserve Forecast</span>
            <span
              className={`font-mono font-bold text-xs ${
      ...
      {isLeak && alert?.topSensors && alert.topSensors.length > 0 && (
        <div className="mb-3 p-2.5 rounded-md bg-[#09090b]/50 border border-zinc-800/60 text-xs">
          <span className="text-zinc-400 block mb-1.5 font-medium text-[11px]">
          ...
          <div className="grid grid-cols-3 gap-1.5 font-mono text-[11px]">
            {alert.topSensors.map((s) => (
              <div
                key={s.node}
                className="bg-[#000000] px-2 py-1 rounded border border-zinc-800 flex items-center justify-between"
              >
                <span className="text-zinc-400">J{s.node}</span>
      ...
      {isLeak && onAskGPT && (
        <button
          onClick={onAskGPT}
          className="w-full flex items-center justify-between px-3 py-2 rounded-md bg-cyan-950/40 hover:bg-cyan-900/50 border border-cyan-500/30 text-cyan-300 text-xs font-medium transition-all group cursor-pointer"
        >
